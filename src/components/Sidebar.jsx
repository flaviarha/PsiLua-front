...
>
  ...
  {isOpen ? <X size={24} /> : <Menu size={24} />}
      ...
      {/* 📌 Sidebar principal */}
      <div
        className={`fixed left-0 top-0 h-full w-64 bg-white shadow-xl transform transition-transform duration-300 z-40 ${
      ...
      >
        <div className="flex flex-col h-full">
          {/* 📌 Logo da aplicação */}
          <div className="flex items-center space-x-3 p-6 border-b border-black/10">
            {/* Logo com cantos arredondados */}
            <img src="/logo.png" alt="PsiLua" className="w-10 h-10 rounded-lg" />
            <div>
              <span className="text-xl font-bold text-black">PsiLua</span>
              <p className="text-xs text-black/60">Sistema de agendamento psicológico</p>
            </div>
          </div>
          ...
          <div className="p-6 border-b border-black/10">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-black/60 capitalize">{user?.type}</p>
              </div>
            </div>
          </div>
                ...
                <li key={link.to}>
                  <Link
                    ...
                    className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-colors ${
                      isActive(link.to)
                        ? 'bg-accent text-white' // ativo: fundo claro + texto branco
                        : 'text-black/70 hover:text-black hover:bg-black/10' // inativo: texto cinza, hover melhora contraste
                    }`}
          ...
          <div className="p-4 border-t border-black/20">
            <button
              ...
              className="flex items-center space-x-3 w-full px-4 py-3 text-black/70 hover:text-white hover:bg-white/10 rounded-xl transition-colors"
            >
              <LogOut size={20} />
              <span>Sair</span>
            </button>
